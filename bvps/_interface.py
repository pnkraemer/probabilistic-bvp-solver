...
from ._ode_measmods import from_second_order_ode, from_ode
...
def probsolve_bvp(
    ...
    msrvs = _RandomVariableList(
        [measmod.forward_realization(m, t=t)[0] for m, t in zip(evaluated_kalman_posterior.mean, candidate_locations)]
    )
    ...
    while True:
        ...
        quotient = stopcrit_bvp.evaluate_quotient(errors, evaluated_posterior.mean)
        # mask = np.linalg.norm(quotient, axis=1) > np.sqrt(bvp_dim)
        mask = np.linalg.norm(errors, axis=1) > np.median(np.linalg.norm(errors, axis=1))
