...
initial_grid = np.linspace(bvp.t0, bvp.tmax, 1500)
...
for N in [2**10, 2**9, 2**8, 2**7, 2**6, 2**5, 2**4]:
    ...
    ax[i].fill_between(t, 1e-16*np.ones_like(t), error_estimates_std ** 2, color="black", alpha=0.2)
    ax[i].set_ylabel("Log-Variance", fontsize="x-small")
    ax[i].semilogy(
        t,
        true_error ** 2,
    ...
    ax[i].set_ylabel("Solution", fontsize="x-small")
    ...
    ax[i].set_ylabel("Residual", fontsize="x-small")


    i = next(idx)

    # Next row: Log-residual and uncertainties
    ...
    ax[i].fill_between(t, error_estimates_res_mean[:, 0] ** 2, error_estimates_res_mean[:, 0] ** 2 + error_estimates_res_std[:, 0] ** 2,color="blue", alpha=0.2)
    ...
    ax[i].set_ylim(LOG_YLIM)
    ax[i].set_ylabel("Log-Residual", fontsize="x-small")
    i = next(idx)

    for x in kalman_posterior.locations:
        ax[i].axvline(x, color="black", linewidth=0.25)
